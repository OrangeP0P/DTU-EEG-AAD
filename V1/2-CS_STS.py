...
from DataLoaderCS_STS import read_source_data
from DataLoaderCS_STS import read_target_data
...
#######################################选取对应的Channel和设置############################################
'''DTU: Channel = 66 (64 × EEG + 2 × Ref), Samples = 3000'''
source_subject_ID = '1'
target_subject_ID = '2'
#######################################选取对应的数据集############################################
...
# source_data = source_data[:, :, Channel_list, :]
# target_data = target_data[:, :, Channel_list, :]
# validation_data = validation_data[:, :, Channel_list, :]

source_data = Data.TensorDataset(torch.from_numpy(source_data.astype(np.float32)),
                                 torch.from_numpy(source_label.astype(np.float32)))
...
plt.savefig('Figure/Train/Cross_' + str(Cross_Mission+1) + '/'
            + 'CLF_LOSS/' + source_subject_ID + ' CLF Loss')
plt.close()
...
plt.savefig('Figure/Train/Cross_' + str(Cross_Mission+1) + '/'
            + 'MMD_LOSS/' + source_subject_ID + ' MMD Loss')
plt.close()
...
plt.savefig('Figure/Train/Cross_' + str(Cross_Mission+1) + '/'
            + 'TOTAL_LOSS/' + source_subject_ID + ' TOTAL LOSS')
plt.close()
